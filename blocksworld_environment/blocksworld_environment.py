...
class Measure(Action):
    ...
    def describe(self):
        if self.env.noise:
            return "You can measure the height of any block X with <measure X>. The measurement may be noisy. Multiple measurements can be taken to get a better idea of the true height. There is no limit to the number of measurements you can take. Take as many measurements as you need to be confident in your estimate. You can only measure one block at a time."
        else:
